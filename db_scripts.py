...
def add_questions():
    ...
    open()
    cursor.executemany('''INSERT INTO questions (questions, answer, wrong1, wrong2, wrong3) VALUES (?, ?, ?, ?, ?)''', questions)
    conn.commit()
    close()
...
def get_question_after(question_id=0, quiz_id=1):
    open()
    cursor.execute('''SELECT quiz_content.id, questions.question, questions.answer, questions.wrong1, questions.wrong2, 
    questions.wrong3
    FROM quiz_content, questions
    WHERE quiz_content.id == questions.id
    AND quiz_content.id > ? AND quiz_content.quiz_id == 
    OREDER BY quiz_content.id''', [question_id, quiz_id])
